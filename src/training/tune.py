...
# tune.py lives at src/training/tune.py
#   parents[0] = src/training/
#   parents[1] = src/
#   parents[2] = project root
_ROOT = Path(__file__).resolve().parents[2]

# shared.py is a sibling in the same directory — add it to path
...
def load_registry(registry_path: Path) -> dict:
    """Load the full model_registry.yaml, returning an empty dict if missing."""
...
def update_registry(
    ...
    """
    Update only the target dataset's champion entry in model_registry.yaml.
    All other dataset entries are preserved untouched.
    """
    registry = load_registry(registry_path)

    # Ensure the dataset key exists with a champion sub-key
    ...
    print(f" Registry updated: {registry_path}  "
          f"[{dataset} champion: {backbone_name} RMSE={best_rmse:.4f}]")
...
def objective(
    ...
    backbone_name = trial.suggest_categorical(
        "backbone", ["lstm", "gru", "tcn", "transformer"]
    )
    ...
    if backbone_name in ("lstm", "gru"):
        ...
        # d_model must be divisible by nhead — prune invalid combinations
    ...
    # Track the global best and save if improved
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", choices=["FD001", "FD002", "FD003", "FD004"],
        default="FD001",
    )
    ...
    if "global_best_rmse" not in study.user_attrs:
        study.set_user_attr("global_best_rmse", float("inf"))

    study.optimize(
        lambda trial: objective(trial, train_df, val_df, train_cfg, args.dataset),
